...
export const HamburgerMenu = () => {
  ...
  return (
    <section className="hamburger-menu flex h-full w-3/5 items-center justify-end lg:hidden">
      <i className="fas fa-bars cursor-pointer text-2xl text-slate-50" onClick={() => setOpen(!open)} />
      {open &&
        <HamburgerAnimated className="absolute top-24 z-50 h-fit w-fit rounded-xl bg-slate-50">
          {Menus.map((menu, i) =>
            <Link to={`/${menu.toLowerCase().replace(" ", "-")}`} key={i} className="grid h-full w-full cursor-pointer place-items-center px-10 py-5 font-semibold text-slate-950 transition-all duration-300 ease-in-out first:rounded-t-xl last:rounded-b-xl hover:bg-slate-200 first:hover:rounded-t-xl last:hover:rounded-b-xl">{menu}</Link>
          )}
        </HamburgerAnimated>
      }
    </section>
